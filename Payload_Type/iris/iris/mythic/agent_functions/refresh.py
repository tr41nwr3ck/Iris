...
### Assistant:
    ...
    def query_graphql(self, token):
        ...
        # Get task output
        self.query_task_output(reader)

        return reader.load_data(task_query, variables={})
    
    def query_task_output(self, reader: IrisGraphQLReader):
        task_query = """query GetTasks{task{id agent_task_id callback_id command_name completed display_params display_id is_interactive_task operator_id operation_id responses{id response_escape}parent_task_id}}"""
        reader.load_data(task_query, variables={})
